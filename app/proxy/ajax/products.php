...
$meta_attributes = [
	'type_category' => ['map_from' => 'product_type', 'map_to' => 'category', 'values' => $db->query("SELECT type_id, category_id FROM product_type_categories_codes t")->fetchAll(PDO::FETCH_COLUMN | PDO::FETCH_GROUP)],
	'scent_family' => ['map_from' => 'scent', 'map_to' => 'scent_family', 'values' => $db->query("SELECT scent_id, family_id FROM scent_family_codes t")->fetchAll(PDO::FETCH_COLUMN | PDO::FETCH_GROUP)],
];
...
//			$attributes_by_variant[$variant_id][$meta_attribute['map_to']] = [];
		...
		print_r($product_attributes[$map_from]);
